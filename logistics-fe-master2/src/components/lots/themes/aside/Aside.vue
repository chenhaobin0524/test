...
export default {
    ...
    methods: {
        ...
        getElementInfo(element) {
            let info = '';
            const menuElement = element.closest('.el-menu-item');
            if (menuElement) {
                info = menuElement.innerText;
            }
            return info;
        },
        ...mapMutations({
    ...
    mounted() {
        ...
        this.$nextTick(() => {
            ...
            aside.$el.addEventListener('click', (e) => {
                const elementInfo = this.getElementInfo(e.target);
                // 大数据详情点击
                bigDataLog.sendDetailMessage({
                    model_name1: elementInfo
                });
