            ...
            <el-table-column v-for="(item, index) in columnsList"
                ...
                <template slot-scope="scope">
                    ...
                    <div v-else-if='item.type === "detail"'>
                        <el-button size="mini" type='text' @click.stop='$emit("btnEvent", "detail", scope.row)' @dblclick.stop="">{{item.buttonName || '明细'}}</el-button>
                    </div>
                    <template v-else>
...
export default {
    ...
    methods: {
        tableRowClassName({ row }) {
            if (this.selectedRowsData.includes(row)) {
        ...
        btnEvent(selection, event) {
            ...
            if (event !== 'edit') {
                ...
                this.$refs.moduleTable.toggleRowSelection(selection);
            }
        },
