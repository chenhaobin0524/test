<template>
    <div id="mPane3">
        <m-search-list :popPrefix="popPrefix" ref="searchList" :searchId="searchId"
            :list="config.searchList" :searchTotalTitle="searchTotalTitle" :initSearchInfo="initSearchInfo"
        ...
        <el-row type="flex" class="row-bg" justify="space-between">
          ...
          <el-col :span="12">
            <m-page ref="page" :current-page="currentPage" :pageData="config.page" :total="pageTotal" @on-page-change="pageChange" @on-size-change="sizeChange"></m-page>
          </el-col>
        </el-row>
    </div>
</template>
...
export default {
    ...
    data() {
        return {
            ...
            resourceCode: '',
            searchId: 'searchListConfig',
            pageID: 'columnsConfig'
        };
    },
    props: {
        config: Object,
    ...
    methods: {
        ...
        btnEvent(event, data) {
            if (event === 'view') {
                ...
                this.$emit(`${event}-btn`, data);
            }
        },
        photosTable(data) {
            this.$emit('photos-btn', data);
        },
        detailTable(data) {
            this.$emit('detail-btn', data);
        },
        uploadTable(data) {
            this.$emit('upload-btn', data);
        },
        // 表格中按钮触发事件
        checkTable(data) {
            this.$emit('check-btn', data);
        ...
        editorTable(data) {
            this.$emit('editor-btn', data);
        },
        removeTable(data) {
            this.$emit('remove-btn', data);
        },
        dispatchTable(data) {
            this.$emit('dispatch-btn', data);
        },
        saveData(data) {
        ...
        sizeChange(size) {
            this.config.page.pageSize = size;
        ...
        async getTableTitle() {
            if (this.config.configuration) {
                ...
                return getList.get(this.configurationUrl, pramas).then(data => {
                    if (data.data && data.data.configNameCn) {
        ...
        trajectoryTable(data) {
            this.$emit('trajectory-btn', data);
        },
        btnClick(val, index, row) {
            this.$emit('btnClick', val, index, row);
        }
    },
    mounted() {
