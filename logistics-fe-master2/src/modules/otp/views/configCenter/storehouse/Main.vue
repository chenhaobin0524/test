    ...
    <paper>
        <m-pane
            ...
            <div slot="page-module-btn">
                <span v-has="'05070001'"><el-button type="primary" @click="redomet">删 除</el-button></span>
                <span style="margin-left: 10px;" v-has="'05070002'"><el-button type="primary" @click="saveData">保 存</el-button></span>
            </div>
        </m-pane>
    </paper>
</template>
...
import { getWhConfigs, batchUpdateWhConfig, batchDeleteWhConfig } from '../../../api/configCenter';
...
export default {
    ...
    methods: {
        async fetchData() {
            const res = await getWhConfigs(this.searchParams);
            if (res) {
        ...
        async saveData() {
            ...
            const selectData = this.selectData.map(item => {
                ...
                const { customerCode, whCode, logisitcMode, version } = item;
                return { customerCode, whCode, logisitcMode, version, ...obj };
            });
            const res = await batchUpdateWhConfig(selectData);
            if (res.code === '0') {
        ...
        async redomet() {
            ...
            messageBox(this).then(async () => {
                const res = await batchDeleteWhConfig(selectData);
                if (res.code === '0') {
