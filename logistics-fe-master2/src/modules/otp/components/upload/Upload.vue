<template>
    <div>
        ...
        <el-dialog title="查看" width="50%" :visible.sync="dialogVisible" append-to-body v-if='showFileList'>
            <div :style="{textAlign: 'center'}">
                <img height="300px" :src="dialogImageUrl">
            </div>
        </el-dialog>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        onExceed(files, fileList) {
            this.$message.warning(`单次上传不能超过${this.limit}张`);
            return this.cancelRequest();
        },
        onPreview(file) {
            if (this.fileList.length > 0) {
                this.dialogImageUrl = file.url + '?certification=' + file.certification;
            } else {
                this.dialogImageUrl = file.response.data.downUrl + '?certification=' + this.certification;
            }
            if (this.onPreviewDialog) this.dialogVisible = true;
        ...
        beforeUpload(file) {
            ...
            if (!this.onBeforeUpload(file)) return this.cancelRequest();
