    ...
    <div class="gwms-tags-view-container" id="gwms-tags-view-container">
        <div ref="scrollPane" class="tags-view-wrapper">
            <div class="all-tags-inner" v-if="isShowTabs">
                <router-link v-for="tag in visitedViews.slice(0, 5)" ref="tag" :class="isActive(tag)?'active':''"
                    ...
                    <span class="tags-view-item">{{$t((tag.meta && tag.meta.title) || tag.name)}}{{tag.orderNo}}<span class="el-icon-close" @click.prevent.stop="closeSelectedTag(tag)" /></span>
                </router-link>
        ...
        <div class="all-tags" ref="allTags">
            ...
            <el-dropdown v-if='hasUploadList'>
                <i class="icon-gerenwenjian file-list"></i>
    ...
    methods: {
        ...
        addViewTags() {
            ...
            if (name) {
                ...
                if (this.visitedViews.length > 5 && !temp) {
                    let tempArr = [];
                    tempArr = Utils.ArrMove(this.visitedViews, this.visitedViews.length - 1, 5);
                    tempArr = Utils.ArrMove(tempArr, 0, tempArr.length - 1);
                    this.COVER_VISITED_VIEW(tempArr);
                    this.allTags = this.closeAllTag.concat(this.visitedViews.slice(5));
                    // this.$router.push({ path: item.path, query: item.query, fullPath: item.fullPath });
                }
            }
            this.allTags = this.closeAllTag.concat(this.$store.getters.visitedViews.slice(5));
            return false;
        },
        moveToCurrentTag() {
        ...
        closeSelectedTag(view) {
            ...
            this.$store.dispatch('delView', view).then(({ visitedViews }) => {
                ...
                this.allTags = this.closeAllTag.concat(visitedViews.slice(5));
            });
        },
        closeOthersTags() {
