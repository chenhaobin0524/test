<template>
    <div class="otp-table">
        ...
        <el-table class="el-table-box" v-else ref='moduleTable' :data="tableData" border tooltip-effect='dark' v-loading="loading"
            ...
            <el-table-column type="selection" width="40" :reserve-selection="reserveSelection" :fixed="tableConfig.fixedSeletionColumn?true:false" v-if="tableConfig.selectBtn"></el-table-column>
...
export default {
    ...
    methods: {
        ...
        getRowKeys(row) {
            if (!row.expandsRandomKey) row.expandsRandomKey = this.reserveSelection ? row[this.unique] : row[this.unique] + Math.random().toString().substr(-10);
