                ...
                <template slot-scope="scope">
                    <div style="display: flex;" v-if="scope.row.edit !== EDIT_TYPE.CANCEL && item.require">
                        ...
                        <div v-else-if='item.type === "detail"'>
                            <el-button size="mini" type='text' @click.stop='btnEvent(scope.row, "editdetail")' @dblclick.stop="">
                                {{item.buttonName || '明细'}}
                            </el-button>
                        </div>
                        <el-input
                    ...
                    <template v-else>
                        ...
                        <div v-else-if='item.type === "detail"'>
                            <el-button size="mini" type='text' @click.stop='btnEvent(scope.row, "detail")' @dblclick.stop="">
                                {{item.buttonName || '明细'}}
                            </el-button>
                        </div>
                        <template v-else>
...
export default {
    ...
    methods: {
        ...
        addRow() {
            ...
            // 新增返回值，用于手动修改新增的数据
            this.btnEvent(obj, 'addRowResponse');
        },
    ...
    computed: {
        ...
        mustFill() {
            return (item) => {
                return item.mustFill ? 'column-must-fill' : '';
            };
        }
    }
};
