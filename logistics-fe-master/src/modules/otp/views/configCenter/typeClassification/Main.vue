...
import { getProjectClassifyConfigs, batchUpdateProjectClassifyConfig, batchDeleteProjectClassifyConfig } from '../../../api/configCenter';
export default {
    name: 'type',
    ...
    methods: {
        async fetchData() {
            const res = await getProjectClassifyConfigs(this.searchParams);
            if (res) {
        ...
        async saveData() {
            ...
            const selectData = this.selectData.map(item => {
                ...
                const { whCode, businessMode, sourceSystem, projectClassify, version } = item;
                return { whCode, businessMode, sourceSystem, projectClassify, version, ...obj };
            });
            const res = await batchUpdateProjectClassifyConfig(selectData);
            if (res) {
        ...
        async redomet() {
            ...
            if (selectData.length > 0) {
                messageBox(this).then(async () => {
                    const res = await batchDeleteProjectClassifyConfig(selectData);
                    if (res) {
